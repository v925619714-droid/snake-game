...
describe('botDirection', () => {
  ...
  test('flood-fill на торе: со сквозными стенами карман у края открыт → идёт к своей еде', () => {
    const s = duel({
      snakes: [
        [{ x: 1, y: 0 }, { x: 1, y: 1 }],
      ...
      dirs: ['right', 'up'],
      pending: ['right', 'up'],
      foods: [{ pos: { x: 0, y: 0 }, color: 1 }], // своя еда вверху
    });
    // С wrap «карман» {0,1},{0,0} соединяется с открытым полем (через верхний край) →
    // места достаточно, и еда сверху → бот идёт up.
    expect(botDirection(s, 1, noMistake)).toBe('up');
  });
