...
// Жирная еда: периодически появляется ЗОЛОТАЯ еда; кто съел — сразу вырастает на
// FAT_GROWTH сегментов. Доступна любому игроку (нейтральная).
export const FAT_GROWTH = 3;
export const FAT_FOOD_EVERY = 60; // как часто (тиков) спавнить жирную еду, если её нет
...
// Стены СКВОЗНЫЕ (wrap-around): уход за край → выход с противоположной стороны (как во
// всех режимах). Предохранитель финиша (вместо прежнего «сжатия арены»): если за столько
// тиков живых всё ещё >1 — матч завершаем, побеждает самая длинная змейка. ~5 мин (150мс/тик).
export const PARTY_MAX_TICKS = 2000;
...
export function partyStep(state: PartyState, rng: () => number = Math.random): PartyState {
  ...
  // Еда под головой → рост (обычная +1, жирная +FAT_GROWTH).
  ...
  } else if (tick >= PARTY_MAX_TICKS) {
    // Предохранитель финиша (стены сквозные): затянулось → побеждает самая длинная змейка.
    status = 'over';
    const rest = [...aliveIdx].sort((a, b) => newSnakes[a].length - newSnakes[b].length);
    winner = rest[rest.length - 1] ?? -1;
    finalPlacements = [...placements];
    for (const i of rest) if (!finalPlacements.includes(i)) finalPlacements.push(i);
  }
