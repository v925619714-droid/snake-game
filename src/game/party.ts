...
// Жирная еда: периодически появляется ЗОЛОТАЯ еда (нейтральная); кто съел — вырастает
// на число ОСТАВШИХСЯ в живых игроков (см. partyStep). Не чаще раза в FAT_FOOD_EVERY тиков.
export const FAT_FOOD_EVERY = 60;
...
// Стены сквозные (wrap-around): уход за край → выход с противоположной стороны.
// Матч идёт «до смерти», БЕЗ таймера — раунд кончается, когда остаётся ≤1 живой.
// Корпоративная игра — best-of: первый, кто выиграл PARTY_WINS_NEEDED раундов, побеждает.
export const PARTY_WINS_NEEDED = 2;

export type PartyStatus = 'playing' | 'roundOver' | 'matchOver';
export type PartyCrashCause = 'wall' | 'self' | 'opponent' | 'head_on';

export interface PartyFood {
  pos: Point; // нейтральная еда: её ест любой игрок (+1)
  fat?: boolean; // жирная еда (золотая): +FAT_GROWTH роста, появляется периодически
}
...
export interface PartyState {
  ...
  tick: number;
  status: PartyStatus;
  roundWinner: number; // слот выжившего в раунде; -1 = ничья
  matchWinner: number; // слот победителя МАТЧА (набрал PARTY_WINS_NEEDED); -1 = не определён
  roundWins: number[]; // длины N — выигранных раундов на игрока
  round: number; // номер текущего раунда (с 1)
  placements: number[]; // порядок выбывания В ТЕКУЩЕМ РАУНДЕ: [0] — первый выбывший … [last] — выживший
...
export function ensureFoods(state: PartyState, rng: () => number = Math.random): PartyFood[] {
  ...
  let have = foods.filter((f) => !f.fat).length; // жирная еда не считается за обычную
  if (have >= target) return foods;
  ...
  const free: number[] = [];
  for (let y = 0; y < state.board; y++) {
    for (let x = 0; x < state.board; x++) {
      const k = y * state.board + x;
      if (!occupied.has(k)) free.push(k);
    }
  }
  while (have < target && free.length > 0) {
...
// Свободная клетка поля (для спавна жирной еды), не на змеях/еде.
function freePartyCell(state: PartyState, rng: () => number): Point | null {
  const occupied = new Set<number>();
  for (let i = 0; i < state.snakes.length; i++) {
    if (!state.alive[i]) continue;
    for (const p of state.snakes[i]) occupied.add(key(p, state.board));
  }
  for (const f of state.foods) occupied.add(key(f.pos, state.board));
  const free: number[] = [];
  for (let y = 0; y < state.board; y++) {
    for (let x = 0; x < state.board; x++) {
      const k = y * state.board + x;
  ...
  return { x: k % state.board, y: Math.floor(k / state.board) };
}
...
export function partyNewMatch(count: number, rng: () => number = Math.random): PartyState {
  ...
  const base: PartyState = {
    ...
    roundWinner: -1,
    matchWinner: -1,
    roundWins: Array.from({ length: n }, () => 0),
    round: 1,
    placements: [],
...
// Следующий раунд best-of: все возрождаются (свежие позиции), счёт побед раундов сохраняется.
export function partyNextRound(state: PartyState, rng: () => number = Math.random): PartyState {
  if (state.status === 'matchOver') return state;
  const n = state.snakes.length;
  const { snakes, dirs } = spawnSnakes(n, state.board);
  const base: PartyState = {
    ...state,
    snakes,
    dirs,
...
export function partyStep(state: PartyState, rng: () => number = Math.random): PartyState {
  ...
  // Жирная еда в командной игре растёт от числа ОСТАВШИХСЯ в живых (больше игроков — крупнее).
  const aliveCount = state.alive.filter(Boolean).length;
  ...
  const heads: Point[] = state.snakes.map((s, i) =>
    state.alive[i] ? wrap(nextPoint(s[0], pending[i]), board) : s[0],
  );

  // Еда под головой → рост (обычная +1, жирная +кол-во живых игроков).
  const growBy: number[] = new Array(n).fill(0);
  const eaten = new Set<number>();
  for (let i = 0; i < n; i++) {
    if (!state.alive[i]) continue;
    const f = state.foods.find((fd) => pointsEqual(fd.pos, heads[i]));
    if (f) {
      growBy[i] = f.fat ? aliveCount : 1; // жирная: +кол-во живых игроков
      eaten.add(key(f.pos, board));
    }
  }

  // Крах: своё тело / тело чужого (стены сквозные — смерти об стену нет).
  const crashed: boolean[] = new Array(n).fill(false);
  const causes: (PartyCrashCause | null)[] = [...state.causes];
  for (let i = 0; i < n; i++) {
    if (!state.alive[i]) continue;
    const h = heads[i];
    const ownBody = growBy[i] > 0 ? state.snakes[i] : state.snakes[i].slice(0, -1);
  ...
  const newSnakes = state.snakes.map((s, i) => {
    ...
    const ns = [heads[i], ...s];
    if (growBy[i] === 0) ns.pop();
    else if (growBy[i] > 1) {
      const tail = s[s.length - 1];
      for (let k = 1; k < growBy[i]; k++) ns.push({ ...tail });
    }
    return ns;
  });
  const scores = state.scores.map((sc, i) => sc + (state.alive[i] && !crashed[i] ? growBy[i] : 0));
  ...
  // Еда: убрать съеденную, дополнить обычную до цели + периодически добавить жирную.
  const remaining = state.foods.filter((f) => !eaten.has(key(f.pos, board)));
  const mid: PartyState = { ...state, snakes: newSnakes, alive, board, foods: remaining };
  let foods = ensureFoods(mid, rng);
  if (tick % FAT_FOOD_EVERY === 0 && !foods.some((f) => f.fat)) {
    const cell = freePartyCell({ ...mid, foods }, rng);
    if (cell) foods = [...foods, { pos: cell, fat: true }];
  }

  // Конец раунда: остался ≤1 живой (БЕЗ таймера — играем до смерти). Выживший берёт раунд;
  // набрал PARTY_WINS_NEEDED побед → победитель матча. Иначе — следующий раунд (partyNextRound).
  ...
  if (aliveIdx.length <= 1) {
    roundWinner = aliveIdx.length === 1 ? aliveIdx[0] : -1;
    ...
    if (roundWinner >= 0) roundWins[roundWinner] += 1;
    if (roundWinner >= 0 && roundWins[roundWinner] >= PARTY_WINS_NEEDED) {
      status = 'matchOver';
...
export function partyKill(state: PartyState, slot: number): PartyState {
  ...
  return { ...state, alive, placements: finalPlacements, status, roundWinner, matchWinner, roundWins };
}
